// MAPPING CVE - Code FINAL avec SCORING INTELLIGENT + CVE VIRTUELS 2026
...
console.log(
  `🔍 MAPPING CVE: ${ext.length} extensions, ${pool.length} autres, ${items.length} total`
);
...
ext.forEach((item, i) => {
  ...
  console.log(
    `       url=${item.json?.url}, originalUrl=${item.json?.extensionData?.originalUrl}`
  );
});
...
for (const e of ext) {
  ...
  // Nouveau seuil et logique renforcée
  const MIN_CONFIDENCE = 8; // seuil abaissé
  const requiredCriticalTokens = 1;
  const threatTypes = [
    "malicious",
    "unwanted",
    "phishing",
  ...
  const isThreatTypeMatch =
    bestMatch &&
    bestMatch.matchingTokens.some((t) => threatTypes.includes(t.toLowerCase()));

  // Bonus si le type de menace est détecté
  let matchMethod = "token_correlation";
  if (isThreatTypeMatch) {
    matchMethod = "token_correlation_threat";
  }

  if (
    bestMatch &&
    bestMatch.conf >= MIN_CONFIDENCE &&
    bestMatch.criticalMatches >= requiredCriticalTokens
  ) {
    console.log(
      `  🎯 ✅ CVE ATTRIBUÉ (assoupli): ${bestMatch.c.id} (confiance: ${bestMatch.conf})`
    );
    ...
    out.push(enrichedExtension);
    console.log(
      `  ✅ Extension enrichie (tokens assoupli) avec extensionId="${originalExtensionId}"`
    );
  } else {
    console.log(
      `  🛡️ ❌ AUCUN MATCH pertinent (score: ${
        bestMatch?.conf || 0
      }) - CVE virtuel généré`
    );
...
console.log(
  "🟦 DEBUG - Résultats mapping_cve.js :",
  JSON.stringify(out, null, 2)
);
// On retourne uniquement les résultats enrichis (extensions traitées)
return out;
